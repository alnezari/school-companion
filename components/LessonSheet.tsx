...
type BodyProps = { period: Period; entry: Entry | null; done: boolean; lang: Lang; d: Dict; onClose: () => void; onDone: (done: boolean, feeling?: "easy" | "ok" | "hard") => void };

/** The lesson itself: header, the teacher's words, and the Done / faces / undo actions. Used inside an expanded card. */
export function LessonBody({ period, entry, done, lang, d, onClose, onDone }: BodyProps) {
...
export function LessonSheet(props: BodyProps) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, transition: { duration: DUR.base, ease: EASE_MOVE } }} transition={{ duration: DUR.base }}
      className="fixed inset-0 z-40 flex items-end justify-center bg-black/40 sm:items-center" onClick={props.onClose}>
