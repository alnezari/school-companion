...
export default function SettingsPage() {
  ...
  return (
    <main className="min-h-dvh px-4 py-4">
      <form onSubmit={save} className="mx-auto max-w-sm">
        <div className="flex items-center gap-2"><Link href="/parent" className="rounded-full border border-line bg-white px-3 py-1.5 text-sm font-semibold">←</Link><h1 className="font-display text-xl font-extrabold">{d.settings}</h1></div>
        <label className="mt-4 block text-sm font-medium">{d.childName}</label>
        ...
        <button className="mt-4 w-full rounded-xl bg-accent py-2 font-semibold text-white">{saved ? d.saved : d.save}</button>
        <button type="button" onClick={logout} className="mt-6 w-full rounded-xl border border-line py-2 text-sm font-semibold text-ink-2">{d.logout}</button>
      </form>
    </main>
  );
}
