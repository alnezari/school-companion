        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {slots.map(({ period, entry }) => {
              ...
              const done = !!progress[pkey(day, period.slot)]?.done_at;
              ...
              return (
                <button key={period.slot} onClick={() => setOpenSlot(period.slot)}
                  className={`rise relative flex min-h-36 flex-col rounded-3xl border-4 bg-white p-4 text-start shadow-sm transition active:scale-[.98] ${done ? "opacity-80" : ""}`}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="ms-auto text-2xl">{done ? <span className="pop inline-block">⭐</span> : <span className="text-line">☆</span>}</span>
                  </div>
                  <div className="mt-2 font-display text-lg font-extrabold leading-tight">{meta[lang]}</div>
